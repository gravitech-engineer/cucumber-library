...
// */
// char calcTemperature(double T, double adc_T)
// //
// {
...
{
    // Wire.begin(sdaPin,sclPin);
    // Wire.begin(GPIO_NUM_4, GPIO_NUM_5, 400000);
    i2c_config_t i2c_conf;
...
char BMP280::readBytes(unsigned char *values, char length)
{
    ...
    // }
    // return (0);
    // Wire.readRegister(BMP280_ADDR, values, 1, values, length);
    i2c_cmd_handle_t _cmd;
    esp_err_t ret;
...
char BMP280::writeBytes(unsigned char *values, size_t length)
{
    ...
    // else
    //     return (0);
    // uint32_t _len = uint32_t(length);
    // Wire.writeRegister(BMP280_ADDR, values, length);
    i2c_cmd_handle_t _cmd;
    esp_err_t ret;
